...
struct Args {
    ...
    #[arg(short, long, requires("bench"), default_value_t = 3)]
    sample_count: usize,
}
...
fn print_time(secs: f64) {
    if secs > 1.0 {
        ...
        print!("{:>6.2}ms", secs * 1_000.0);
    } else {
        print!("{:>6.2}μs", secs * 1_000_000.0);
    }
}
    ...
    if args.bench {
        ...
        if let Some(day) = args.day {
            if let Some(part) = args.part {
                ...
                let samples = bench(day, 1, sample_count);
                benchmarks.push(Benchmark::from_samples(
                    format!("Day {day:2}, part 1"),
                    samples,
                ));

                let samples = bench(day, 2, sample_count);
                benchmarks.push(Benchmark::from_samples(
                    format!("Day {day:2}, part 2"),
                    samples,
                ));
            }
        } else {
