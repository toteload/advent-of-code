    ...
    fn push_button_part2(&mut self) -> bool {
        ...
        while let Some((source, range, pulse)) = signals.pop_front() {
            for idx in &self.groups[range.start..range.end] {
                ...
                match m {
                    ModuleKind::Dummy if pulse == PULSE_LO => detected = true,
                    ModuleKind::FlipFlop(x) if pulse == PULSE_LO => {
                        let y = *x * -1;
                        *x = y;
...
pub fn part2(input: &str) -> u64 {
    ...
    // TODO I solved this one sort of manually, by evaluating parts of the circuit and then
    // calculating the total amount of clicks necessary. To solve this automatically you need to
    // recognize the important subgraphs, evaluate them and then calculate the answer.
